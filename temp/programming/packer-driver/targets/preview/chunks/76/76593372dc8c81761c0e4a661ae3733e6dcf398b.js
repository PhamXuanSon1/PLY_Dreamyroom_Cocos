  ...
  return {
    ...
    execute: function () {
      ...
      _export("Slot", Slot = (_dec = ccclass('Slot'), _dec2 = property(_crd && Thing === void 0 ? (_reportPossibleCrUseOfThing({
        ...
        init() {
          var sps = this.node.getComponentsInChildren(Sprite);
          this.frame = sps[0];
        ...
        moveToPos(pos, pos2) {
          ...
          var lpos = this.node.position.clone();
          var t = this;
          var time = pos.clone().subtract(lpos).length() / 3000;
        ...
        setThingFrame(thing) {
          ...
          this.thing = thing;
          var size = thing.getComponent(UITransform).contentSize.clone();
          var thingScale = thing.node.worldScale.clone();
          var t = v2(size.width * thingScale.x, size.height * thingScale.y);
          var frameSize = this.thingys[0].getComponent(UITransform).contentSize.clone();
          frameSize = new Size(45, 45);
          var thingyScale = this.thingys[0].node.worldScale.clone();
          var ty = v2(frameSize.width * thingyScale.x, frameSize.height * thingyScale.y);
          var mul = 0.9;
          var ratio2 = v2(t.x / ty.x, t.y / ty.y);
          var ratio = 1 / Math.max(ratio2.x, ratio2.y) * mul;
          var thingySize = v2(t.x / thingyScale.x * ratio, t.y / thingyScale.y * ratio);
        ...
        setSlotThing(thing, linear, callback) {
          if (linear === void 0) {
            linear = false;
          }
          ...
          var sk = thing.node.parent.parent.getComponent(sp.Skeleton);
          ...
          var star = (_crd && World === void 0 ? (_reportPossibleCrUseOfWorld({
            error: Error()
          ...
          var thingScale = thing.node.worldScale.clone();
          var t = v2(size.width * thingScale.x, size.height * thingScale.y);
          var frameSize = this.thingys[0].getComponent(UITransform).contentSize.clone(); // frameSize = new Size(40, 50);
          ...
          var ratio2 = v2(t.x / ty.x, t.y / ty.y);
          var ratio = 1 / Math.max(ratio2.x, ratio2.y) * mul;
          var scale = thing.node.scale.clone().multiplyScalar(ratio);
          var up = 0;
          var dTime = up / 500;
          ...
          var pos = this.node.worldPosition.clone();
          var s = thing.node.worldPosition.clone();
          var src = s.clone().add(v3(0, up, 0));
          var local = thing.node.parent.inverseTransformPoint(v3(), src);
          var dir = pos.clone().subtract(src);
          var time = dir.length() / 3000;
          ...
          var temp = v3();
          var des = v3();
          var slot = this.thingys[this.added].node; // const slot = (this.thingys[this.added] || this.thingys[0]).node;

          var tick = this.ticks[this.added];
          var before = this.added + 0;
          this.added += 1;
          var delay = 0.0;
          thing.onHightlight();
            ...
            if (thing.firstIndex >= 0) {
              ...
              tween(thing.node).to(dTime, {
                ...
                onUpdate(target, ratio) {
                  ...
                  temp = (_crd && Ulis === void 0 ? (_reportPossibleCrUseOfUlis({
                    error: Error()
                  }), Ulis) : Ulis).lerpParabola(src, des, ratio, _height, 0.5);
                  thing.node.worldPosition = temp;
                }
        ...
        setBoxThing(thing, linear) {
          ...
          var thingScale = thing.node.worldScale.clone();
          var t = v2(size.width * thingScale.x, size.height * thingScale.y);
          var frameSize = this.frame.getComponent(UITransform).contentSize.clone();
          var thingyScale = this.frame.node.worldScale.clone();
          var ty = v2(frameSize.width * thingyScale.x, frameSize.height * thingyScale.y);
          var mul = 0.9;
          var ratio2 = v2(t.x / ty.x, t.y / ty.y);
          var ratio = 1 / Math.max(ratio2.x, ratio2.y) * mul;
          ...
          if (linear) {
            ...
            var pos = this.node.worldPosition.clone();
            var src = thing.node.worldPosition.clone();
            var dir = src.clone().subtract(pos.clone());
            var time = dir.length() / 3000;
            time = (_crd && ui === void 0 ? (_reportPossibleCrUseOfui({
              error: Error()
            }), ui) : ui).width < (_crd && ui === void 0 ? (_reportPossibleCrUseOfui({
            ...
            var _pos = this.node.worldPosition.clone();

            var s = thing.node.worldPosition.clone();

            var _src = s.clone().add(v3(0, up, 0));

            var local = thing.node.parent.inverseTransformPoint(v3(), _src);

            var _dir = _pos.clone().subtract(_src);

            var _time = _dir.length() / 3000;

            _time = (_crd && ui === void 0 ? (_reportPossibleCrUseOfui({
              error: Error()
            }), ui) : ui).width < (_crd && ui === void 0 ? (_reportPossibleCrUseOfui({
              error: Error()
            }), ui) : ui).height ? _time : _time * 0.35;
            ...
            if (thing.firstIndex >= 0) {
              thing.offAnim();
              thing.node.worldPosition = _slot.node.worldPosition.clone();
              thing.node.layer = Math.pow(2, Layers.nameToLayer("UI_2D"));
              ...
              this.moveTween = tween(thing.node).to(dTime, {
                ...
                onUpdate(target, ratio) {
                  _des = _slot.node.worldPosition.clone();
                  _temp = (_crd && Ulis === void 0 ? (_reportPossibleCrUseOfUlis({
                    error: Error()
                  }), Ulis) : Ulis).lerpParabola(_src, _des, ratio, _height2, 0.5);
                  thing.node.worldPosition = _temp;
                }
        ...
        onSlot(thing) {
          ...
          if (this.things.length == 3) {
            var movingNode = this.things.map(t => t.node);
...
//# sourceMappingURL=76593372dc8c81761c0e4a661ae3733e6dcf398b.js.map