  ...
  return {
    ...
    execute: function () {
      ...
      _export("MatchSprite", MatchSprite = (_dec = ccclass('MatchSprite'), _dec2 = property(Node), _dec3 = property(JsonAsset), _dec4 = property({
        ...
        setScale() {
          var sprites = this.thingNode.getComponentsInChildren(Sprite);
          sprites.forEach(sprite => {
            var sc = sprite.node.scale.clone();
            var ux = sc.x > 0 ? 1 : -1;
            var uy = sc.y > 0 ? 1 : -1;
            sprite.node.scale = v3(ux, uy, 1);
            var ui = sprite.getComponent(UITransform);
            var size = ui.contentSize.clone();
        ...
        setSpine() {
          var spines = this.thingNode.getComponentsInChildren(sp.Skeleton);
          spines.forEach((s, i) => {
            var skins = s.skeletonData.skeletonJson["skins"];
            var index = skins.findIndex(sk => sk.name == s.node.name);
        ...
        matchSpine() {
          ...
          var nodeOrders = [];
          var spines = this.thingNode.getComponentsInChildren(_crd && NodeOrder === void 0 ? (_reportPossibleCrUseOfNodeOrder({
            error: Error()
          }), NodeOrder) : NodeOrder);
          var things = this.json.json;
          var keys = Object.keys(things);

          var _loop = function _loop() {
            var key = keys[i];
            var thing = things[key];
            var sps = spines.filter(s => s.node.name == key);
            thing.forEach((t, i) => {
              if (!sps[i]) {
                sps[i] = instantiate(sps[i - 1].node).getComponent(_crd && NodeOrder === void 0 ? (_reportPossibleCrUseOfNodeOrder({
                  error: Error()
                }), NodeOrder) : NodeOrder);
                sps[i].node.parent = _this.thingNode;
              }

              sps[i].node.position = v3(t.x, t.y, t.z).multiplyScalar(_this._mul);
              sps[i].order = t.order;
              sps[i].skin = t.skin;
          ...
          nodeOrders.forEach((no, i) => {
            ...
            no.index = i;
            var t = no.getComponent(sp.Skeleton);
            var skins = t.skeletonData.skeletonJson["skins"];
            var index = skins.findIndex(s => s.name == no.skin);
        ...
        matchSprite() {
          ...
          var nodeOrders = [];
          var sprites = this.thingNode.getComponentsInChildren(_crd && NodeOrder === void 0 ? (_reportPossibleCrUseOfNodeOrder({
            error: Error()
          }), NodeOrder) : NodeOrder);
          var things = this.json.json;
          var keys = Object.keys(things);
          ...
          var _loop2 = function _loop2() {
            ...
            var sps = sprites.filter(s => s.node.name == key);
            thing.forEach((t, i) => {
              if (!sps[i]) {
                // return;
                try {
                  ...
                  sps[i].node.parent = _this2.thingNode;
                } catch (error) {
                  console.log(key);
                  return;
                }
              ...
              sps[i].node.position = v3(t.x, t.y, t.z).multiplyScalar(_this2._mul);
              sps[i].order = t.order;
              nodeOrders.push(sps[i]);
            });
          };
...
//# sourceMappingURL=2cbefcc3572a637ec6bce2fc7a36f619773ac75f.js.map