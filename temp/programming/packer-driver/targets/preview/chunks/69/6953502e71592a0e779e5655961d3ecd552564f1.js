System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2"], function (_export, _context) {
  ...
  var _reporterNs, __checkObsolete__, __checkObsoleteInNamespace__, Node, Sprite, SpriteFrame, UITransform, Vec2, Vec3, _decorator, misc, rect, resources, v2, v3, pc, ui, _dec, _class, _descriptor, _class3, _crd, ccclass, property, Ulis, cEasing, Zoom;
    ...
    execute: function () {
      ...
      _export("default", Ulis = ccclass(_class3 = class Ulis {
        ...
        static lerpParabola(A, B, t, height, peak) {
          ...
          var curve = t * (1 - t) / (peak * (1 - peak)); // tránh vượt quá do sai số
        ...
        static getBox(b) {
          var ut1 = b.getComponent(UITransform);
          var scale1 = b.getWorldScale();
          var w1 = ut1.width * scale1.x;
          var h1 = ut1.height * scale1.y;
          var wpos1 = b.getWorldPosition();
          var box = rect(wpos1.x - w1 / 2, wpos1.y - h1 / 2, w1, h1);
          return box;
        }
        ...
        static simplifyPath(path) {
          ...
          for (var i = 1; i < path.length - 1; i++) {
            ...
            var curr = path[i];
            var next = path[i + 1];
            var v1 = new Vec2(curr.x - prev.x, curr.y - prev.y);

            var _v = new Vec2(next.x - curr.x, next.y - curr.y); // cross 2D


            var cross = v1.x * _v.y - v1.y * _v.x; // cùng hướng => bỏ điểm giữa
        ...
        static getWpos(lpos, node) {
          var matrix = node.parent.getWorldMatrix();
      ...
      _export("cEasing", cEasing = function cEasing(type, param) {
        ...
        switch (type) {
          ...
          case 'bounceIn':
            return k => {
              var bounceFactor = Math.max(0.00001, param);
          ...
          case 'bounceOut':
            return k => {
              if (k <= 0) return 0;
              if (k >= 1) return 1;
              var bounceFactor = Math.max(1, Math.round(param));
              var adjustedK = k * bounceFactor;
...
//# sourceMappingURL=6953502e71592a0e779e5655961d3ecd552564f1.js.map