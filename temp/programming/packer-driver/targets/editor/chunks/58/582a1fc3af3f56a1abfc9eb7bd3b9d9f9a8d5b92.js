  ...
  return {
    ...
    execute: function () {
      ...
      _export("InputManager", InputManager = (_dec = ccclass('DreamyInputManager'), _dec2 = executeInEditMode(true), _dec3 = property({
        ...
        setupAllLayers() {
          const roots = this.targetRoots && this.targetRoots.length > 0 ? this.targetRoots : [this.node];
          const targetLayer = this.customLayer;
          let count = 0;
          ...
          console.log(`[DreamyInputManager] Đã gán Layer (${targetLayer}) cho ${count} node.`);
        ...
        onTouchStart(ev) {
          ...
          const worldPos = InputManager.toWorld(ev); // Broadcast: ItemManager cần biết mọi lần chạm (reset idle timer, first-click,
          ...
          const candidates = this.handlers.filter(h => h.node && h.node.isValid && h.node.activeInHierarchy && h.hitTest(worldPos)).sort((a, b) => a.inputPriority - b.inputPriority || -InputManager.compareRenderOrder(a.node, b.node));
...
//# sourceMappingURL=582a1fc3af3f56a1abfc9eb7bd3b9d9f9a8d5b92.js.map