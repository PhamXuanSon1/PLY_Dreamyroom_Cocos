...
export class InputManager extends Component {
    ...
    private onTouchStart(ev: EventTouch): void {
        ...
        const candidates = this.handlers
            .filter((h) => h.node && h.node.isValid && h.node.activeInHierarchy && InputManager.isNodeInteractable(h.node) && h.hitTest(worldPos))
            .sort((a, b) => (a.inputPriority - b.inputPriority)
                || (-InputManager.compareRenderOrder(a.node, b.node)));
    ...
    private onTouchMove(ev: EventTouch): void {
        ...
        this.captor.onPointerMove?.(InputManager.toWorld(ev), ev);
    }
    ...
    private onTouchEnd(ev: EventTouch): void {
        ...
        const c = this.captor;
        this.captor = null;
        c?.onPointerUp?.(InputManager.toWorld(ev), ev);
